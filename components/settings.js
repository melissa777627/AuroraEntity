...
const USER_ICON_CATEGORIES = [
  { label: '✨ จักรวาล', icons: ['🌙','☀️','⭐','🌟','💫','✨','🌈','🔥','💧','🌿','❄️','⚡','🌊','🌸','🌺','🌻','🌹','🌷','🍀','🌼','🪻','🌾','🍃','🌑','🌕','🌠','🪐','🌌','🌤️','🌪️'] },
  { label: '🐾 สัตว์', icons: ['🦋','🐉','🦅','🦁','🐯','🦊','🐺','🦝','🐻','🐼','🐨','🦘','🦙','🦔','🐸','🐊','🐢','🐍','🦈','🦭','🦦','🐙','🦀','🦜','🦚','🦩','🦢','🕊️','🦉','🦇','🐝','🐞','🦋','🐠','🐬','🐳'] },
  { label: '🔮 ลึกลับ', icons: ['🔮','💎','🪄','🎭','👁️','🗡️','🛡️','🏺','🧿','🔯','🪬','🌀','⚗️','🌒','🌘','⚜️','🗝️','🕯️','📿','🪤','🪩','🎪','🎡','🎠','🌃'] },
  { label: '🍓 น่ารัก', icons: ['🍓','🍰','🎀','🍒','🍑','🧁','🍦','🫐','🍇','🍎','🍭','🍬','🎁','🧸','💖','💕','💗','💝','🫶','🎵','🎶','🌸','🪄','🍡','🍩','🧃','🫧','🌈','🐾','🎊'] }
];
...
export function renderSettings(container) {
  ...
  container.innerHTML = `
    <div class="page">
      ...
      <div class="settings-page">
        ...
        <div class="profile-section">
          <div class="profile-section-title">ไอคอนของคุณ</div>
          <p style="font-size:0.78rem;color:var(--text-soft);margin-bottom:10px">ไอคอนที่เลือกจะแสดงเป็น profile bubble ของคุณในแชท</p>
          <div style="display:flex;gap:12px;align-items:flex-start">
            <div id="user-icon-preview" style="font-size:2.5rem;width:60px;height:60px;display:flex;align-items:center;justify-content:center;border-radius:50%;background:var(--bg-secondary);border:2px solid var(--accent);cursor:pointer;flex-shrink:0" title="คลิกเพื่อเปลี่ยน">
              ${s.userAvatar || '🌸'}
            </div>
            <div style="flex:1">
              <p style="font-size:0.78rem;color:var(--text-soft);margin:0">คลิกที่ไอคอนเพื่อเลือกใหม่</p>
            </div>
          </div>
          <div id="user-icon-picker" style="display:none;margin-top:10px">
            ${USER_ICON_CATEGORIES.map(cat => `
              <div style="font-size:0.72rem;color:var(--text-soft);margin:8px 0 4px;font-weight:600;letter-spacing:0.04em">${cat.label}</div>
              <div class="icon-picker-grid">${cat.icons.map(ic => `
                <div class="icon-option${ic === (s.userAvatar || '🌸') ? ' selected' : ''}" data-icon="${ic}">${ic}</div>
              `).join('')}</div>
            `).join('')}
          </div>
        </div>
  ...
  // User icon picker
  const userIconPreview = container.querySelector('#user-icon-preview');
  const userIconPickerEl = container.querySelector('#user-icon-picker');
  userIconPreview.addEventListener('click', () => {
    userIconPickerEl.style.display = userIconPickerEl.style.display === 'none' ? 'block' : 'none';
  });
  container.querySelectorAll('#user-icon-picker .icon-option').forEach(el => {
    el.addEventListener('click', () => {
      container.querySelectorAll('#user-icon-picker .icon-option').forEach(x => x.classList.remove('selected'));
      el.classList.add('selected');
      userIconPreview.textContent = el.dataset.icon;
      userIconPickerEl.style.display = 'none';
    });
  });
  ...
  container.querySelector('#save-settings-btn').addEventListener('click', () => {
    const newSettings = {
      ...
      userAvatar: container.querySelector('#user-icon-picker .icon-option.selected')?.dataset.icon || s.userAvatar || '🌸'
    };
