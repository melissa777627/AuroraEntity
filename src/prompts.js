...
export function buildBackstagePrompt(entities, cards, tones, gimmick, interjection) {
  ...
  const entityBlocks = entities.map((e, i) => {
    ...
    const pronoun3rdHint = `คำเรียก keeper ลับหลัง: อนุมานจากที่เรียกตรงๆ ว่า "${callUser}"${personality ? ` + บุคลิก "${personality}"` : ''} — เลือก 1 คำแล้วยึดคำนั้นตลอด ไม่เปลี่ยน ไม่หลากหลาย`;
    return `[${e.id}] ${e.name}
persona: ${persona.slice(0, 200)}
${pronoun3rdHint}
ไพ่ลับ → ${card.nameTH || card.name} (${card.keywords?.slice(0,3).join(', ') || ''}) — ใช้พลังงานนี้ guide อารมณ์และการ react ของ${e.name} อย่างละเอียดอ่อน ห้ามพูดถึงไพ่โดยตรง`;
  }).join('\n\n');
...
✓ TONE คือพลังงานและ dynamic ของบทสนทนา ไม่ใช่ topic ของทุกประโยค — ไม่จำเป็นต้องพูดถึง keeper ในทุกข้อความ
...
CLIFFHANGER — บังคับมี:
ทุก episode ต้องมี cliffhanger อย่างน้อย 1 ครั้ง — ข้อความที่จบกลางคัน / เปิดเรื่องน่ารู้แล้วยังไม่เฉลย / ถามแล้วรอ reaction
เช่น: "รู้มั้ยว่าเมื่อกี้มัน—" / "มีเรื่องนึงที่ยังไม่ได้บอก..." / "แล้วเธอจะทำยังไงกับ..." / คำถามที่อีกคนยังไม่ตอบ
✓ ใช้ได้ 1–2 ครั้งต่อ episode ห้ามมากกว่านี้
✗ ข้อความธรรมดาที่บทสนทนาไหลปกติห้าม mark

ตอบเป็น JSON array เท่านั้น ห้ามมี text นอก JSON:
[{"entityId":"id","text":"...","cliffhanger":true},...]`;
}
